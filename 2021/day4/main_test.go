...
var testNumbers []int = []int{
	7, 4, 9, 5, 11, 17, 23, 2, 0, 14, 21, 24, 10, 16, 13, 6, 15, 25, 12, 22, 18, 20, 8, 19, 3, 26, 1,
}

// getBoards returns an instance of the test boards
...
func TestPart1(t *testing.T) {
	expected := 4512
	boards := getBoards() //get a copy of the test boards

	result := part1(testNumbers, boards)
	if result != expected {
...
func TestPart2(t *testing.T) {
	...
	result := part2(testNumbers, boards)
	if result != expected {
		t.Errorf("got: %d, want: %d", result, expected)
	}
}
